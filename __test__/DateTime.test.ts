...
test(`解析字符串 "${str}" `, () => {
	...
	expect(dateTime.toString('yyyy-mm-dd hh:ii:ss')).toBe(str);
});

let str2 = '1543373878615';
test(`解析时间戳 "${str2}"`, () => {
	const dateTime = DateTime.parse(str2);
	expect(dateTime.toString('yyyy-mm-dd hh:ii:ss')).toBe('2018-11-28 10:57:58');
});
...
test(`解析含有时间的字符串 "${str4}"`, () => {
	...
	expect(dateTime.toString('yyyy-mm-dd hh:ii:ss')).toBe('2015-12-31 16:00:38');
});
